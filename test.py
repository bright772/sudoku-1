...
class Board:
    ...
    def __init__(self):
        ...
        self.row_picks_list = [[False for i in range(
            self._rows_size)] for i in range(self._rows_size)]
        self.row_picks_count = [0 for i in range(self._rows_size)]
        self.column_picks_list = [[False for i in range(
            self._columns_size)] for i in range(self._columns_size)]
        self.column_picks_count = [0 for i in range(self._columns_size)]
        self.square_picks_list = [[False for i in range(
            self._squares_size)] for i in range(self._squares_size)]
        self.square_picks_count = [0 for i in range(self._squares_size)]
        self._board = [[Cell() for j in range(self._columns_size)]
    ...
    def is_possible_to_place_element(self, x_index, y_index, elem):
        if(x_index >= self._rows_size or x_index < 0 or y_index >= self._columns_size or y_index < 0):
            raise ModelError("row or column selection out of range (x:{},y:{})".format(
                x_index + 1, y_index + 1))
        if(self._board[x_index][y_index].value is not -1):
            raise ModelError("already an element at row {}, column{})".format(
                x_index + 1, y_index + 1))
        if(self.row_picks_list[x_index][elem]):
            raise ModelError("element {} already in use in the selected row {}".format(
                elem + 1, x_index + 1))
        if(self.column_picks_list[y_index][elem]):
            raise ModelError("element {} already in use in the selected column {}".format(
                elem + 1, y_index + 1))
        square_index = self.get_square_index(x_index, y_index)
        if(self.square_picks_list[square_index][elem]):
            raise ModelError("element {} already in use in the square {}".format(
                elem + 1, square_index + 1))
        return True
    ...
    def is_possible_to_delete_element(self, x_index, y_index):
        ...
        if(self._board[x_index][y_index].value is -1):
            raise ModelError("empty cell at row {}, column{})".format(
                x_index + 1, y_index + 1))
        return True
...
class Controller():
    ...
    def execute(self):
        print(
            'Welcome to a new game of Sudoku !\n\nUse row -1 and column -1 to cancel a play')
        while(not self.model.end()):
            print(self.model.board)
            option_selected = int(
                input('Select your next play:\n1. Place number\n2. Delete number\n0. Exit\n-->'))
            if(option_selected is 1):
                row_selected = int(
                    input('Type the row of your next play [1, 9]-->'))
                column_selected = int(
                    input('Type the column of your next play [1, 9]-->'))
                if(row_selected is not -1 and column_selected is not -1):
                    element_selected = int(
                        input('Type the element of your next play [1, 9]-->'))
                    try:
                        """ Prepares row, column and element for the range used at place_element() [0, 8] """
                        self.model.place_element(
                            row_selected - 1, column_selected - 1, element_selected - 1)
                ...
                if(row_selected is not -1 and column_selected is not -1):
                    try:
                        ...
                        self.model.delete_element(
                            row_selected - 1, column_selected - 1)
                    except ModelError as e:
                        print(e)
